...
class WorkerCleansingUseCaseTest {
    ...
    fun `cleanse idle workers successfully`() = runTest {
        ...
        val worker1 = io.dave.shortener.domain.model.Worker(id = 1L, workerNum = 1L, workerName = "worker-1", status = WorkerStatus.ACTIVE, updatedAt = threshold.minusMinutes(1))
        val worker2 = io.dave.shortener.domain.model.Worker(id = 2L, workerNum = 2L, workerName = "worker-2", status = WorkerStatus.ACTIVE, updatedAt = threshold.minusMinutes(2))
