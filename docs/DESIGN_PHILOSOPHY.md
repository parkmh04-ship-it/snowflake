...
*   **Coroutines 관용적 스타일**: `subscribe()` 같은 Java 스타일 리액티브 체인을 제거하고, `awaitSingleOrNull()`을 사용하여 순차적이고 직관적인 비동기 흐름을 구현했습니다.
*   **지속성 및 안정성**: 애플리케이션 재배포 시에도 캐시 유지 및 장애 시 데이터 손실 최소화.

### 왜 도메인 모듈을 물리적으로 분리하는가?
`snowflake-shorter-domain`을 별도 Gradle 모듈로 분리하여 **물리적인 컴파일 장벽**을 세웠습니다.
*   **오염 차단**: 도메인 엔티티에 JPA(`@Entity`)나 Spring 어노테이션이 섞이는 것을 원천적으로 방지합니다.
*   **추상화 강제**: 인프라 기술을 사용하려면 반드시 도메인 포트(Port)를 인터페이스로 정의하도록 강제하여 결합도를 낮춥니다. (DIP 실현)
