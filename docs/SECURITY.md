...
* **알고리즘**: 고정 윈도우 카운터 (Fixed Window Counter).
* **기술 스택**: Redis + Lua Script (원자성 보장).
* **임계치**: 기본 IP당 1분간 100회 요청 허용.
* **작동 계층**: WebFilter (`RateLimitFilter`) 계층에서 라우터 진입 전 차단.
* **장애 대응**: Redis 장애 시 fail-open(정상 요청 차단 방지).

### 📝 Lua Script 상세

Redis의 원자적 연산을 위해 다음과 같은 Lua 스크립트를 사용합니다:

1. `INCR`로 IP별 카운터를 1 증가.
2. 카운터가 1이면(윈도우 첫 요청) `EXPIRE`로 윈도우(초) TTL 설정.
3. 카운터가 임계치 이하이면 허용, 초과하면 거부.
