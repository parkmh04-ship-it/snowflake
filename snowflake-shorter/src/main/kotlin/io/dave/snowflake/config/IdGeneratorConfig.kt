...
import io.dave.snowflake.config.metrics.MonitoredIdGenerator
...
class IdGeneratorConfig(
    ...
    /**
     * 애플리케이션 시작 시점에 필요한 워커 ID를 할당받아 AssignedWorkerInfo 빈을 생성합니다. 동기적으로 실행되어야 하는 Configuration 빈 생성을
     * 위해 Repository의 @Transactional 메소드를 직접 호출합니다.
     */
    @Bean
    @Profile("!test")
    fun assignedWorkerInfo(): AssignedWorkerInfo {
        ...
        logger.info {
            "Successfully assigned ${assignedWorkerIds.size} worker IDs to instance [$instanceId]: $assignedWorkerIds"
        }
        return AssignedWorkerInfo(instanceId, assignedWorkerIds)
    }

    /**
     * 할당받은 워커 ID들을 기반으로 PooledIdGenerator 빈을 생성합니다. 각 SnowflakeIdGenerator는 MonitoredIdGenerator로
     * 래핑되어 메트릭을 수집합니다.
     */
    @Bean
