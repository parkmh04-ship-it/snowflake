...
interface FailedEventRepository : JpaRepository<FailedEventEntity, Long> {
    ...
    fun findByStatusOrderByFailedAtAsc(status: FailedEventStatus): List<FailedEventEntity>

    /** 재시도 가능한 이벤트를 조회합니다. (PENDING 상태이고 재시도 횟수가 MAX_RETRY_COUNT 미만) */
    @Query(
            """
        SELECT e FROM FailedEventEntity e 
        WHERE e.status = 'PENDING' 
        AND e.retryCount < :maxRetryCount 
        ORDER BY e.failedAt ASC
    """
    )
    fun findRetryableEvents(@Param("maxRetryCount") maxRetryCount: Int): List<FailedEventEntity>
